...
const [prev, next] = await queryContent('/cv')
...
const { data, error } = await useAsyncData(cleanPath, async () => {
	...
	let article = queryContent('/cv').where({ _path: cleanPath }).findOne();
	return {
...
const goPost = () => clearError({ redirect: '/cv' });
</script>
		...
		<ContentDoc>
			<template v-slot="{ doc }">
				<article>
					...
					<span class="cv"><ContentRenderer :value="doc" /></span>
				</article>
